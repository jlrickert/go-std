...
// SandboxOption is a function used to modify a Fixture during construction.
...
// SandboxOptions holds optional settings provided to NewFixture.
...
// NewSandbox constructs a Fixture and applies given options. Cleanup is
...
func WithWd(path string) SandboxOption {
	return func(f *Sandbox) {
		f.t.Helper()
		f.env.Setwd(path)
	}
...
func (f *Sandbox) AbsPath(rel string) string {
	f.t.Helper()
...
func (f *Sandbox) ResolvePath(path string) string {
	f.t.Helper()
	p, _ := std.ExpandPath(f.Context(), path)
...
func (f *Sandbox) WriteJailFile(path string, data []byte, perm os.FileMode) error {
	f.t.Helper()
	if f.Jail == "" {
...
func (f *Sandbox) MustWriteJailFile(path string, data []byte, perm os.FileMode) {
	f.t.Helper()
...
func (f *Sandbox) DumpJailTree(maxDepth int) {
	f.t.Helper()
	if f.Jail == "" {
...
func (f *Sandbox) Advance(d time.Duration) {
	f.t.Helper()
	f.clock.Advance(d)
}

// Now returns the current time from the fixture test clock.
func (f *Sandbox) Now() time.Time {
	f.t.Helper()
	return f.clock.Now()
}

func (f *Sandbox) Getwd() string {
	f.t.Helper()
	wd, _ := f.env.Getwd()
	return wd
}
